...
from models import Patient, Doctor, Specialization, Room, Appointment, DoctorSpecialization
...
def patients():
    if request.method == "GET":
        query = db.session.query(Patient.id, Patient.name, Patient.address, Patient.telephone)
        results = query.all()
        patients_list = [
            {
                "id": result.id,
    ...
    if request.method == "POST":
        data = request.json
        new_patient = Patient(name=data['name'], address=data['address'], telephone=data['telephone'])
...
def specializations():
    if request.method == "GET":
        ...
        return {
            ...
            'results': [{'id': spec.id, 'name': spec.name} for spec in specializations_list]
        }
...
def appointments():
    ...
    if request.method == "POST":
        ...
        new_appointment = Appointment(
            ...
            time=datetime.strptime(data['time'], '%H:%M:%S').time(),  # Convert string to time
            date=datetime.strptime(data['date'], '%Y-%m-%d').date(),  # Convert string to date
...
def appointment(id):
    ...
    if request.method == "PUT":
        ...
        appointment.time = datetime.strptime(data['time'], '%H:%M:%S').time()  # Convert string to time
        appointment.date = datetime.strptime(data['date'], '%Y-%m-%d').date()  # Convert string to date
